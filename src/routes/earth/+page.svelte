...
<main class="earth-page">
	<section class="top">
		<div class="title">
			<p class="tag">Blue Home</p>
			<h1>Earth</h1>
			<p class="subtitle">Our living planet with oceans and air.</p>
		</div>
		<nav class="jump">
					<button
						type="button"
	...
	<section class="earth-layout">
		<div class="earth-core" aria-hidden="true">
	...
	<section class="panels">
		<article class="panel" data-active={activeSection === 'id-overview'} id="id-overview">
			<h2>Overview</h2>
			<p>Our living planet with oceans and air.</p>
			<p>Only known world with life.</p>
		</article>
...
<style>
	...
	.earth-page { min-height: 100svh; padding: clamp(1.5rem, 4vw, 3rem); display: grid; gap: clamp(1.2rem, 2.4vw, 2.2rem); }
	...
	.earth-layout { position: relative; min-height: 320px; display: grid; place-items: center; }
	.earth-core { position: relative; width: min(240px, 40vw); aspect-ratio: 1; border-radius: 50%; display: grid; place-items: center; filter: drop-shadow(0 20px 50px rgba(255, 170, 120, 0.35)); }
	.earth-sphere { position: absolute; inset: 0; border-radius: 50%; background: radial-gradient(circle at 30% 30%, rgba(255, 235, 220, 0.9), transparent 40%), radial-gradient(circle at 70% 60%, rgba(255, 170, 120, 0.6), transparent 50%), radial-gradient(circle at 50% 50%, #f4caa2, #dd8a5c 60%, #a05135 100%); animation: slowspin 22s linear infinite; }
	.earth-halo { position: absolute; inset: -8%; border-radius: 50%; background: radial-gradient(circle, rgba(255, 170, 120, 0.35), transparent 70%); filter: blur(6px); animation: pulse 4s ease-in-out infinite; }
	...
	@media (max-width: 900px) { .top { grid-template-columns: 1fr; } .jump { grid-template-columns: repeat(3, minmax(0, 1fr)); } .fact { display: none; } .earth-layout { min-height: 240px; } }
</style>
