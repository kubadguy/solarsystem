...
<main class="makemake-page">
	...
	<section class="makemake-layout">
		<div class="makemake-core" aria-hidden="true">
			<div class="makemake-sphere"></div>
			<div class="makemake-halo"></div>
		</div>
	...
	<section class="panels">
		<article class="panel" data-active={activeSection === 'id-overview'} id="id-overview">
			<h2>Overview</h2>
			<p>Kuiper Belt dwarf planet.</p>
			<p>Discovered in 2005.</p>
		</article>
...
<style>
	...
	.makemake-page { min-height: 100svh; padding: clamp(1.5rem, 4vw, 3rem); display: grid; gap: clamp(1.2rem, 2.4vw, 2.2rem); }
	...
	.makemake-layout { position: relative; min-height: 320px; display: grid; place-items: center; }
	.makemake-core { position: relative; width: min(240px, 40vw); aspect-ratio: 1; border-radius: 50%; display: grid; place-items: center; filter: drop-shadow(0 20px 50px rgba(150, 180, 255, 0.35)); }
	.makemake-sphere { position: absolute; inset: 0; border-radius: 50%; background: radial-gradient(circle at 30% 30%, rgba(220, 235, 255, 0.9), transparent 40%), radial-gradient(circle at 70% 60%, rgba(140, 170, 230, 0.6), transparent 50%), radial-gradient(circle at 50% 50%, #cbd8f2, #7aa0d6 60%, #3c5e9c 100%); animation: slowspin 22s linear infinite; }
	.makemake-halo { position: absolute; inset: -8%; border-radius: 50%; background: radial-gradient(circle, rgba(150, 180, 255, 0.35), transparent 70%); filter: blur(6px); animation: pulse 4s ease-in-out infinite; }
	...
	@media (max-width: 900px) { .top { grid-template-columns: 1fr; } .jump { grid-template-columns: repeat(3, minmax(0, 1fr)); } .fact { display: none; } .makemake-layout { min-height: 240px; } }
</style>
